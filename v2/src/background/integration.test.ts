...
const mockStorageLocal = {
  get: vi.fn((keys: string | string[] | null) => {
    return Promise.resolve(
      typeof keys === "string"
        ...
        : Array.isArray(keys)
          ? keys.reduce(
              (acc, key) => ({ ...acc, [key]: mockStorageLocalData[key] }),
              {}
            )
  ...
  clear: vi.fn(() => {
    Object.keys(mockStorageLocalData).forEach(
      (key) => delete mockStorageLocalData[key]
    )
    return Promise.resolve()
  }),
  onChanged: {
...
const mockStorageSession = {
  get: vi.fn((keys: string | string[] | null) => {
    return Promise.resolve(
      typeof keys === "string"
        ...
        : Array.isArray(keys)
          ? keys.reduce(
              (acc, key) => ({ ...acc, [key]: mockStorageSessionData[key] }),
              {}
            )
  ...
  clear: vi.fn(() => {
    Object.keys(mockStorageSessionData).forEach(
      (key) => delete mockStorageSessionData[key]
    )
    return Promise.resolve()
  })
}
...
const mockIdle = {
  ...
  queryState: vi.fn(
    (interval: number, callback: (state: chrome.idle.IdleState) => void) => {
  ...
  onStateChanged: {
    ...
    removeListener: vi.fn(
      (callback: (newState: chrome.idle.IdleState) => void) => {
        idleStateChangeListeners = idleStateChangeListeners.filter(
...
const mockAlarmsAPI = {
  ...
  onAlarm: {
    ...
    removeListener: vi.fn((callback: (alarm: chrome.alarms.Alarm) => void) => {
      alarmListeners = alarmListeners.filter(
        (listener) => listener !== callback
      )
    })
  }
...
function createTestScreenTimeData(
  daysAgo: number,
  domain: string,
  time: number
): ScreenTimeData {
...
describe("Background Services Integration Tests", () => {
  ...
  describe("Scenario 1: Full Tracking Lifecycle", () => {
    it("should track a complete session from start to idle and persist data", async () => {
      ...
      await storageManager.addTimeToCurrentDay(
        "example.com",
        120,
  ...
  describe("Scenario 2: Service Worker Restart Recovery", () => {
    ...
    it("should handle corrupted tracking state gracefully", async () => {
      ...
      mockStorageSessionData.trackingState = {
        corrupted: true,
        invalid: "data"
      }
  ...
  describe("Scenario 3: Idle Detection During Tracking", () => {
    it("should pause tracking on idle and resume on active", async () => {
      ...
      expect(beforeIdleData[yearWeek][dateKey]["stackoverflow.com"].time).toBe(
        30
      )
  ...
  describe("Scenario 4: Data Archiving Workflow", () => {
    it("should archive old data (>90 days) and keep recent data", async () => {
      ...
      const recentData2 = createTestScreenTimeData(
        30,
        "stackoverflow.com",
        1800
      )
      ...
      const mixedData = mergeScreenTimeData(
        recentData1,
        recentData2,
        oldData1,
        oldData2
  ...
  describe("Scenario 5: Blocked Domain Operations", () => {
    ...
    it("should handle concurrent add operations", async () => {
      ...
      const duplicateCount = blockedDomains.filter(
        (d) => d === "duplicate.com"
      ).length
  ...
  describe("Scenario 6: Error Recovery", () => {
    it("should handle storage errors gracefully during write", async () => {
      ...
      mockStorageLocal.set.mockRejectedValueOnce(
    ...
    it("should return null/empty data on read errors", async () => {
      ...
      mockStorageLocal.get.mockRejectedValueOnce(
      ...
      mockStorageSession.get.mockRejectedValueOnce(
  ...
  describe("Edge Cases", () => {
    ...
    it("should handle rapid successive state changes", async () => {
      const states: TrackingState[] = [
        {
          currentUrl: "https://site1.com",
          startTime: Date.now(),
          lastActiveTime: Date.now()
        },
        {
          currentUrl: "https://site2.com",
          startTime: Date.now() + 1000,
          lastActiveTime: Date.now() + 1000
        },
        {
          currentUrl: "https://site3.com",
          startTime: Date.now() + 2000,
          lastActiveTime: Date.now() + 2000
        },
    ...
    it("should handle very large datasets", async () => {
      ...
      for (let day = 0; day < 365; day++) {
        const testData = createTestScreenTimeData(
          day,
          `domain${day % 50}.com`,
          Math.random() * 3600
        )
    ...
    it("should handle favicon updates for existing domains", async () => {
      ...
      await storageManager.updateFavicon(
        "nofavicon.com",
        "https://nofavicon.com/icon.png"
      )
      ...
      expect(data[yearWeek][dateKey]["nofavicon.com"].favicon).toBe(
        "https://nofavicon.com/icon.png"
      )
    })
  })
