...
import { IdleDetector, IdleState, IdleDetectorCallbacks } from './idle-detector';
...
describe('IdleDetector', () => {
  ...
  describe('start', () => {
    ...
    it('should handle missing chrome.idle API gracefully', () => {
      ...
      expect(loggerWarnSpy).toHaveBeenCalledWith(expect.stringContaining('chrome.idle API not available'));
    });
  });
  ...
  describe('stop', () => {
    it('should remove state change listener', () => {
      ...
      expect(mockChromeIdle.onStateChanged.removeListener).toHaveBeenCalledTimes(1);
