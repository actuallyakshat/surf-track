...
import type { TrackingState, ScreenTimeData } from "../types";
  ...
  describe("Session State Operations", () => {
    ...
    describe("updateTrackingState", () => {
      it("should update partial tracking state", async () => {
        ...
        await storageManager.updateTrackingState({ currentUrl: "https://new.com" });
  ...
  describe("Screen Time Data Operations", () => {
    ...
    describe("addTimeToCurrentDay", () => {
      it("should add time for new domain", async () => {
        ...
        expect(setCall.screenTimeData[yearWeek][dateKey]["example.com"]).toEqual({
          time: 30,
          favicon: "icon.png",
        });
      });
      ...
      it("should add time to existing domain", async () => {
        ...
        const setCall = mockChromeStorage.local.set.mock.calls[0][0];
        expect(setCall.screenTimeData[yearWeek][dateKey]["example.com"].time).toBeGreaterThanOrEqual(50);
      });
      ...
      it("should preserve existing favicon if new one not provided", async () => {
        ...
        const setCall = mockChromeStorage.local.set.mock.calls[0][0];
        expect(setCall.screenTimeData[yearWeek][dateKey]["example.com"].favicon).toBe("old-icon.png");
      });
    });
    ...
    describe("updateFavicon", () => {
      it("should update favicon for domain without one", async () => {
        ...
        const setCall = mockChromeStorage.local.set.mock.calls[0][0];
        expect(setCall.screenTimeData[yearWeek][dateKey]["example.com"].favicon).toBe("new-icon.png");
        vi.useRealTimers();
      });
  ...
  describe('Error Handling & Edge Cases', () => {
    ...
    describe('data integrity', () => {
      it('should handle negative time values', async () => {
        ...
        expect(setCall.screenTimeData[yearWeek][dateKey]['test.com'].time).toBe(-10);
      ...
      it('should handle zero time values', async () => {
        ...
        expect(setCall.screenTimeData[yearWeek][dateKey]['test.com'].time).toBe(0);
    ...
    describe('null and undefined handling', () => {
      ...
      it('should handle empty string favicon', async () => {
        ...
        expect(setCall.screenTimeData[yearWeek][dateKey]['test.com'].favicon).toBe('');
